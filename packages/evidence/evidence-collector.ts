...
const INTERESTING_LOG_LINE =
  /error|fail|timeout|exception|denied|unauthorized|forbidden|econnrefused|enotfound|curl:|exit code|strict mode|locator|50[0234]|401|403|not found|unable to|cannot find|expected:|received:|assertion|killed|no space|module not found|health[- ]check|dashboard|token|service unavailable|connection refused/i;
...
const ERROR_SIGNATURE_DETECTORS: Array<{ id: string; pattern: RegExp }> = [
  ...
  { id: 'CONNECTION_REFUSED', pattern: /connection refused|econnrefused|err_connection_refused/i },
  { id: 'DNS_FAILURE', pattern: /could not resolve host|getaddrinfo enotfound/i },
  { id: 'STRICT_MODE', pattern: /strict mode violation/i },
  { id: 'LOCATOR_FAILURE', pattern: /waiting for locator|locator resolved to|element not found/i },
  { id: 'ASSERTION_MISMATCH', pattern: /expect\(received\)|assertionerror|expected:.+received:/is },
...
export function collectFailureEvidence(input: EvidenceInput): NormalizedFailureEvidence {
  ...
  const combinedLogText = input.logs.map((log) => log.content).join('\n');
  ...
  for (const log of logs) {
    ...
    for (const line of lines) {
      ...
      const matchesFailedStep = primaryFailure
        ...
        : false;
      if (!INTERESTING_LOG_LINE.test(trimmed) && !matchesFailedStep) {
        continue;
      }

      const text = trimmed.slice(0, 240);
      if (seen.has(text)) {
        continue;
      }
